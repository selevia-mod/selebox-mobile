// VideosHiddenGems — low-view videos with strong engagement signals.
// The discovery shelf for content that hasn't trended yet but earns
// its watch-through.
//
// Filtering happens upstream in app/(tabs)/videos.jsx where the
// baseVideos pool is scored by (likes + comments) / max(views, 1)
// and bucketed into the bottom-quartile-views / top-quartile-engagement
// crossover. Doing the math there (once per refresh) instead of in
// this component keeps the FlatList renderer cheap.
...
const VideosHiddenGems = ({ videos = [] }) => {
  ...
  return (
    <View style={{ minHeight: containerHeight }} className="space-y-2">
      <VideosSectionTitle
        title={"Hidden Gems"}
        onSeeAllPress={() => router.push({ pathname: "/(video)/shelf-all", params: { type: "hiddenGems" } })}
      />
      <FlatList
        horizontal
...
export default VideosHiddenGems;
