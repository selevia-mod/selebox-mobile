...
const WithdrawModal = ({ visible, onClose, onConfirm, amount, setAmount, remainingBalance, loading }) => {
  ...
  const { amountNum, platformCost, transferFee, totalReceive } = calculateAmountToReceive(amount, globalSettings);

  const handleAmountChange = (text) => {
    const { totalReceive } = calculateAmountToReceive(text, globalSettings);
      ...
      <View className="flex-1 items-center justify-center" style={{ backgroundColor: theme.backdrop }}>
        <View className="w-11/12 rounded-2xl p-6" style={{ backgroundColor: theme.surfaceElevated }}>
          ...
          <View className="mb-2">
            ...
            <View className="flex-row justify-between">
              <Text style={{ color: theme.textSoft }}>Platform Cost ({`${PLATFORM_COST}`}%)</Text>
              <Text style={{ color: theme.text }}>₱ {platformCost.toFixed(2)}</Text>
            </View>
            <View className="flex-row justify-between">
              <Text style={{ color: theme.textSoft }}>Transfer Fee ({`${TRANSFER_FEE}`}%)</Text>
              <Text style={{ color: theme.text }}>₱ {transferFee.toFixed(2)}</Text>
            </View>
