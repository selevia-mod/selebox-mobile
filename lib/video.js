...
// getVideoViewCount only has a Supabase implementation — Appwrite tracked
// views in a separate videoMetricsCollection that wasn't migrated. When
// USE_SUPABASE_VIDEOS=false (rollback) this falls through to a no-op that
// returns the static value the consumer already has.
export const getVideoViewCount =
  impl.getVideoViewCount ?? (async () => 0);
export const incrementVideoComments = impl.incrementVideoComments ?? appwriteImpl.incrementVideoComments;

// v4 shelves — exposed only on the Supabase impl. Appwrite never had
// these; legacy rollback would lose the shelves entirely (acceptable;
// the dispatcher returns no-ops for the missing impls). Each falls
// through to a noop async fn so callers don't have to feature-detect.
const noopArr = async () => [];
const noopVoid = async () => {};
const noopAnchor = async () => ({ anchor: null, recommendations: [] });
export const tickVideoProgress      = supabaseImpl.tickVideoProgress      ?? noopVoid;
export const fetchVideoProgress     = supabaseImpl.fetchVideoProgress     ?? (async () => 0);
export const fetchContinueWatching  = supabaseImpl.fetchContinueWatching  ?? noopArr;
export const fetchRisingCreators    = supabaseImpl.fetchRisingCreators    ?? noopArr;
export const fetchBecauseYouWatched = supabaseImpl.fetchBecauseYouWatched ?? noopAnchor;
export const fetchFromYourFollowers = supabaseImpl.fetchFromYourFollowers ?? noopArr;
