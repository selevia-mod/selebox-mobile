...
// Required for OAuth callback handling on iOS — completes the auth session
// when the in-app browser closes after Google redirects back to us.
...
//
// IMPORTANT: this hydrator MUST also expose the badge-detection fields
// (`roles[]`, `creator`, `moderator`, `auditor`, `userPlus`, `isWriter`)
// so the badge logic in lib/user-roles.js works for the signed-in user's
// OWN profile screen. Without them, `hasPioneerRole` / `getAssignedRoleKeys`
// see undefined and return no badges, even though `role` is set.
// Mirrors hydrateProfileRow in lib/users-supabase.js (the other-user
// path) — keep these in sync.
const hydrateProfile = (profile, authUser = null) => {
  if (!profile) return null;

  // Derive the badge-detection set. Source of truth precedence:
  //   profiles.roles (text[]) — newer column, may not exist yet
  //   profiles.role  (text)   — single-string fallback
  // Pioneer ALSO accepts the legacy `userPlus` / `user_plus` boolean,
  // matching expandProfileRoleFlags in lib/user-roles.js.
  const rolesArray = Array.isArray(profile.roles)
    ? profile.roles
    : profile.role
      ? [profile.role]
      : [];
  const hasRole = (key) => rolesArray.includes(key) || profile.role === key;
  const isPioneer =
    hasRole("pioneer") || Boolean(profile.userPlus) || Boolean(profile.user_plus);
  ...
  return {
    ...
    // ── Badge-detection flags (consumed by lib/user-roles.js) ──
    creator: hasRole("creator"),
    moderator: hasRole("moderator"),
    auditor: hasRole("auditor"),
...
//
// Lookup order (post-auth_user_id-fix May 2026):
//   1. profiles.auth_user_id = authUser.id  → finds migrated users whose
//      profile.id is still the migration UUID (the new permanent shape).
//   2. profiles.id            = authUser.id → backwards compat for users
//      who claimed BEFORE the auth_user_id migration ran (their profile.id
//      was already mutated to match auth.users.id by the old trigger).
//   3. Email match            → catches the lazy-claim race window AND any
//      migrated user whose auth_user_id backfill missed them.
//   4. Auth-metadata stub     → last resort to keep the app booting.
export const getCurrentSupabaseUser = async () => {
  ...
  const fetchProfile = async () => {
    // Try auth_user_id first (the new linkage column post-fix).
    ...
    if (byAuthUserId.error) {
      // The column may not exist yet on environments that haven't run
      // the migration — fall through to id lookup so the app still works.
      const msg = byAuthUserId.error.message || "";
      if (!msg.includes("auth_user_id")) {
        console.log("[supabase-auth] profile fetch by auth_user_id error:", msg);
      }
    } else if (byAuthUserId.data) {
      return byAuthUserId.data;
    }

    // Backwards compat: users who claimed under the OLD trigger have
    // profile.id = auth.users.id directly.
    const byId = await supabase.from("profiles").select("*").eq("id", authUser.id).maybeSingle();
    if (byId.error) {
      console.log("[supabase-auth] profile fetch by id error:", byId.error.message);
      return null;
    }
    if (byId.data) return byId.data;

    // Last DB attempt: match by email. Catches users whose auth_user_id
    // backfill didn't set their row (e.g., email-case mismatch in
    ...
    if (authUser.email) {
      const byEmail = await supabase
        ...
        .maybeSingle();
      if (byEmail.error) {
        console.log("[supabase-auth] profile fetch by email error:", byEmail.error.message);
...
// Browser-based Google sign-in via Supabase OAuth.
//
// Why browser instead of native: native flow requires an Android OAuth
// client matching the App Signing key SHA-1, which is registered in a
// Google Cloud project we no longer control (runaway dev). Browser flow
// only needs the Web Client + redirect URI (both in our Selebox project),
// bypassing the SHA-1 problem entirely. Slight UX cost: brief Custom Tab
// visible during sign-in, then back to the app.
//
// Redirect scheme uses `talesofsiren` (the existing secondary scheme
// declared in app.json), NOT the primary `appwrite-callback-...` scheme
// which is reserved for legacy Appwrite OAuth flows.
export const signInWithGoogle = async () => {
  const redirectUrl = "talesofsiren://books/auth-callback";
  ...
  if (error) throw error;
  if (!data?.url) throw new Error("Failed to start Google OAuth flow");

  const webResult = await WebBrowser.openAuthSessionAsync(data.url, redirectUrl);
  ...
  if (webResult.type !== "success" || !webResult.url) {
    throw new Error("Google OAuth flow did not complete");
  }

  // Handle both PKCE flow (?code=... in query) and implicit flow
  // (#access_token=... in fragment). Supabase defaults to PKCE in v2.x.
  ...
  // Fall back to implicit flow — tokens in fragment
  ...
  if (!access_token || !refresh_token) {
    throw new Error("Missing tokens or code in Google OAuth callback");
  }
