...
export const unblockUser = async ({ blockerId, blockedUserId }) => {
  ...
  const deletions = res.documents.map((doc) =>
    databases.deleteDocument(appwriteConfig.databaseId, appwriteConfig.userBlocksCollectionId, doc.$id),
  );
...
export const listUserReports = async ({ reporterId }) => {
  ...
  const res = await databases.listDocuments(appwriteConfig.databaseId, appwriteConfig.contentReportsCollectionId, [Query.equal("reporterId", reporterId)]);
