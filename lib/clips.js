...
// Single-clip read-through cache. Same clip gets fetched from notifications
// hydration, post-resource resolution (when a post embeds a clip), and the
// clips screen deep-link. 30s TTL keeps comment/like counts reasonably fresh
// without making the user pay for the same getDocument repeatedly.
const CLIP_CACHE = createTtlCache({ ttlMs: 30 * 1000, maxEntries: 200 });

export const invalidateClipCache = (clipId) => {
  if (clipId) CLIP_CACHE.delete(clipId);
};
...
export const getClip = async ({ ID }) => {
  if (!ID) return null;
  const cached = CLIP_CACHE.get(ID);
  if (cached) return cached;
  const doc = await databases.getDocument(appwriteConfig.databaseId, appwriteConfig.clipsCollectionId, ID);
  if (doc) CLIP_CACHE.set(ID, doc);
  return doc;
};
