...
export const readBookChapter = async ({ userId, bookId, chapterId }) => {
  ...
  try {
    ...
    // Try insert first (first-time read of this chapter for this user).
    // On unique-PK conflict (23505), fall through to a read-modify-write
    // that bumps read_count and last_read_at. The trigger on chapter_reads
    // distinguishes INSERT (always bumps) vs UPDATE (cooldown-gated) on
    // its own — we just have to make sure the row reflects this read.
    const nowIso = new Date().toISOString();
    const { error: insertErr } = await supabase
      .from("chapter_reads")
      .insert({ chapter_id: chapterUuid, user_id: userUuid, read_count: 1, last_read_at: nowIso });

    if (!insertErr) return progressResult;
    if (insertErr.code !== "23505") {
      console.warn("[book-reads-supabase] chapter_reads insert failed:", insertErr.message);
      return progressResult;
    }

    // Row exists — bump read_count and last_read_at. Read-modify-write
    // has a tiny race window (two concurrent re-opens could lose one
    // increment) but back-to-back chapter opens from the same user in
    // the same millisecond don't happen in practice.
    const { data: existing } = await supabase
      ...
      .eq("user_id", userUuid)
      .maybeSingle();
    const nextCount = (existing?.read_count ?? 0) + 1;
    const { error: updateErr } = await supabase
      .from("chapter_reads")
      .update({ read_count: nextCount, last_read_at: nowIso })
      .eq("chapter_id", chapterUuid)
      .eq("user_id", userUuid);
    if (updateErr) {
      console.warn("[book-reads-supabase] chapter_reads update failed:", updateErr.message);
    }
  } catch (e) {
    console.warn("[book-reads-supabase] chapter_reads write threw:", e?.message);
  }
