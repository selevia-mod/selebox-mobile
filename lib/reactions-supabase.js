...
export const toggleReaction = async ({ targetType, targetId, emoji = DEFAULT_EMOJI }) => {
  ...
  // Read existing state to decide whether this is an add / change / remove.
  // SELECT is RLS-permitted (true) so this works even on the anon mobile session.
  ...
  // Same-emoji existing reaction → remove.
  ...
  // submit_reaction is "add or change" — handles both the existing-different
  // and the no-existing case in one round-trip. The RPC is SECURITY DEFINER
  // so it bypasses the auth.uid() = user_id RLS check that rejects mobile's
  // anon-keyed direct inserts.
  ...
  if (error) throw error;
  return { action: existing ? "changed" : "added", emoji };
};
...
export const setReaction = async ({ targetType, targetId, emoji }) => {
  ...
  // submit_reaction handles "no existing" (insert) AND "different emoji"
  // (update) atomically. We don't need a SELECT-then-decide round-trip
  // anymore — one RPC call is enough.
  const { error } = await supabase.rpc("submit_reaction", {
    p_actor_id: me.id,
    p_target_type: targetType,
    p_target_id: String(targetId),
...
// Removes the current user's reaction on a target. Idempotent — the
// submit_unreaction RPC returns deleted=0 if nothing was there.
