...
// Path layout. We persist RELATIVE paths (`offline-videos/<id>-720p/<id>-720p.mp4`)
// in Redux+MMKV, then resolve them to absolute URIs at runtime. Reason: on iOS,
// `FileSystem.documentDirectory` contains the app's container UUID, which the
// OS rotates on every TestFlight/App Store update, OS restore, or reinstall —
// any absolute URI we cached from the old container becomes a dead path.
// (Symptom: "Downloads tab videos stop playing after a few days.")
const OFFLINE_VIDEO_BASE_PATH = "offline-videos/";
const getOfflineVideoBaseUri = () => `${FileSystem.documentDirectory || ""}${OFFLINE_VIDEO_BASE_PATH}`;
...
// Resolve a (possibly persisted) download entry's local file location to a
// fresh absolute URI under the CURRENT documentDirectory. Accepts entries from
// either schema:
//   • Modern: `{ relativePath: "offline-videos/.../*.mp4" }`
//   • Legacy: `{ localUri: "file:///var/.../offline-videos/..." }` — strips the
//     stale container prefix and rebases on today's documentDirectory.
// Returns null when no usable path is present.
export const resolveLocalDownloadUri = (entry) => {
  if (!entry) return null;
  const docDir = FileSystem.documentDirectory || "";

  if (typeof entry.relativePath === "string" && entry.relativePath) {
    return `${docDir}${entry.relativePath}`;
  }

  // Legacy entries stored absolute URIs directly. The container UUID baked
  // into them is stale post-update, so chop everything before the
  // "offline-videos/" segment and re-prefix with the current docDir.
  const candidate = entry.localUri || entry.fileUri || entry.manifestUri;
  if (typeof candidate === "string" && candidate.includes(OFFLINE_VIDEO_BASE_PATH)) {
    const tail = candidate.substring(candidate.indexOf(OFFLINE_VIDEO_BASE_PATH));
    return `${docDir}${tail}`;
  }
...
export const resolveDownloadFolderUri = (entry) => {
  if (!entry) return null;
  const docDir = FileSystem.documentDirectory || "";

  if (typeof entry.relativeFolderPath === "string" && entry.relativeFolderPath) {
    return `${docDir}${entry.relativeFolderPath}`;
  }

  const candidate = entry.folderUri;
  if (typeof candidate === "string" && candidate.includes(OFFLINE_VIDEO_BASE_PATH)) {
    const tail = candidate.substring(candidate.indexOf(OFFLINE_VIDEO_BASE_PATH));
    return `${docDir}${tail}`;
  }
...
const ensureOfflineBaseDir = async () => {
  ...
  await FileSystem.makeDirectoryAsync(getOfflineVideoBaseUri(), { intermediates: true });
};
...
export const getVideoDownloadFolderUri = ({ downloadId, quality }) => {
  ...
  return `${getOfflineVideoBaseUri()}${safeId}-${safeQuality}/`;
};

// Same as getVideoDownloadFolderUri but returns the relative form we persist.
const getVideoDownloadRelativeFolderPath = ({ downloadId, quality }) => {
  const safeId = safeNamePart(downloadId);
  const safeQuality = safeNamePart(quality || "auto");
  return `${OFFLINE_VIDEO_BASE_PATH}${safeId}-${safeQuality}/`;
};
...
export const removeDownloadedVideoFiles = async (entry) => {
  // Resolve to fresh absolute URIs so we don't try to delete paths under a
  // stale iOS container UUID (which would silently no-op and leave files on
  // disk). resolveDownloadFolderUri / resolveLocalDownloadUri handle both
  // schemas (modern relativePath + legacy absolute URI).
  const folderUri = resolveDownloadFolderUri(entry);
  const fileUri = resolveLocalDownloadUri(entry);
...
export const downloadVideoOffline = async ({ video, qualitySetting, selectedHeight, onStatusChange, onProgress }) => {
  ...
  try {
    ...
    const relativeFolderPath = getVideoDownloadRelativeFolderPath({ downloadId, quality: qualityTag });
    const fileName = `${safeNamePart(downloadId)}-${qualityTag}.mp4`;
    ...
    const targetFileUri = `${folderUri}${fileName}`;
    ...
    return {
      ...
      // Relative paths are the source of truth — persisted in Redux/MMKV and
      // resolved against the current FileSystem.documentDirectory at runtime
      // (see resolveLocalDownloadUri / resolveDownloadFolderUri). We still
      // emit the absolute URIs as a convenience for the immediate post-
      // download navigation; consumers should prefer the resolvers.
      relativePath,
      relativeFolderPath,
