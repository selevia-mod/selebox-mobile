...
export const sendChatPushNotification = async ({
  ...
  try {
    ...
    // Secret conversations are silent by design — never fire a push.
    // The recipient sees the message only when they actively open the
    // Secret tab. This is the "no notifications" half of the Secret
    // privacy floor; the unread badge is suppressed separately by
    // useTotalUnreadCount.
    if (conversation.is_secret) return;
