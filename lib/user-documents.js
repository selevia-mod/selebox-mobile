...
const UserDocumentsService = {
  ...
  async fetchPaymentInfo(_userId) {
    ...
    const { data: row, error } = await supabase
      .from("author_kyc")
      .select(
        // `supabase_confirmed_at` powers the one-time "Verify your
        // Payment Info" gate on the Withdraw button. NULL = user
        // hasn't saved their payment info through the new Supabase
        // flow yet (legacy migrated row), so we prompt them to
        // confirm before allowing a withdrawal request.
        "user_id, full_name, date_of_birth, id_document_url, selfie_url, payment_qr_url, signature_url, payment_method, phone, email, address, status, rejection_reason, submitted_at, supabase_confirmed_at",
      )
      .eq("user_id", me.id)
      .maybeSingle();
    ...
    return {
      ...
      // Camel-cased for consumers; null until the user saves their
      // payment info through the new Supabase flow at least once.
      // The Withdraw gate in earnings.jsx reads this; null = block
      // and prompt to verify.
      supabaseConfirmedAt: row.supabase_confirmed_at || null,
    };
  },
};
