...
// Search STICKERS — Giphy's separate endpoint for animated transparent
// PNGs / GIFs intended as decorative overlays. Used by the Moments
// editor's EmojiPickerModal under the "Stickers" tab. Same response
// shape as GIFs (we run it through normalizeGiphyResults) so consumers
// can reuse the same display + drop-on-canvas logic that already
// exists for GIFs in story-preview.jsx.
const GIPHY_STICKERS_BASE = "https://api.giphy.com/v1/stickers";

export const searchGiphyStickers = async (query, { limit = 24 } = {}) => {
  const term = (query || "").trim();
  const key = secrets?.GIPHY_API_KEY;
  if (!key) return [];

  const url = term
    ? `${GIPHY_STICKERS_BASE}/search?api_key=${encodeURIComponent(key)}&q=${encodeURIComponent(term)}&limit=${limit}&rating=pg-13`
    : `${GIPHY_STICKERS_BASE}/trending?api_key=${encodeURIComponent(key)}&limit=${limit}&rating=pg-13`;

  try {
    const res = await fetch(url);
    if (!res.ok) {
      console.log("[giphy stickers] non-OK response:", res.status);
      return [];
    }
    const json = await res.json();
    return normalizeGiphyResults(json);
  } catch (e) {
    console.log("[giphy stickers] fetch failed:", e?.message);
    return [];
  }
};

export const fetchTrendingStickers = ({ limit = 24 } = {}) => searchGiphyStickers("", { limit });
