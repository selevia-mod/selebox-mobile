    ...
    rankingsPoolInflight = (async () => {
      try {
        ...
        while (allEntries.length < target) {
          const queries = [Query.limit(Math.min(BATCH_SIZE, target - allEntries.length)), Query.offset(currentOffset), Query.orderDesc("monthlyReads")];

          const response = await databases.listDocuments(
            secrets.appwriteConfig.databaseId,
            secrets.appwriteConfig.booksReadsCollectionId,
            queries,
            ["book"],
          );
