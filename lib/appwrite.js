...
export const isAppwriteAuthError = (error) => {
  ...
  const message = (error?.message || "").toLowerCase();
  return (
    message.includes("user (role: guests) missing scope") ||
    message.includes("session not found") ||
    message.includes("user is not authorized")
  );
};
...
export const signOut = async () => {
  // Disconnect Stream Chat first
  try {
    await streamClient.disconnectUser();
    console.log("Stream Chat disconnected");
  } catch (error) {
    console.warn("Stream disconnect error during signOut:", error.message);
  }
  ...
  try {
    ...
    console.warn("Failed to clear Stream tokens:", error.message);
  }
...
export const getCurrentUser = async () => {
  // Get user document without Stream Chat
  const userDoc = await getCurrentUserWithoutStream();

  // Connect to Stream Chat (can be called separately now)
  const { jwt } = await account.createJWT();
  const { token, userId } = await StreamService.getStreamToken(jwt);

  await streamClient.connectUser({ id: userId }, token);
  await AsyncStorage.setItem("streamToken", token);
  await AsyncStorage.setItem("streamUserId", userId);
