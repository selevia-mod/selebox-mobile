...
export async function updateUsername(documentId, newUsername) {
  try {
    if (USE_SUPABASE_AUTH) {
      ...
      const { error } = await sb.from("profiles").update({ username: newUsername }).eq("id", documentId);
      if (error) throw new Error(error.message || "Failed to update username in Supabase");
      invalidateUserCache(documentId);
      return { $id: documentId, username: newUsername };
    }
