...
// Hotfix (May 2026, USE_SUPABASE_AUTH=true OTA) — when auth lives on
// Supabase, user.$id is a Supabase UUID and the legacy Appwrite
// userCollection has no document with that id. Every legacy write path
// in this file (updateAvatar / updateBanner / updateUsername /
// updateSelectedRole / updateBio / updateUserExpoPushToken) used to do
// `databases.updateDocument(databaseId, userCollectionId, user.$id, ...)`
// which then 404s ("Document with the requested ID could not be found.").
//
// The fix routes the metadata write to Supabase profiles directly when
// USE_SUPABASE_AUTH is on (auth.uid() = user.id, so RLS WITH CHECK
// passes). The Appwrite Storage upload stays unchanged — the Storage
// session still works for migrated users, and the buckets (avatars,
// banners, posts, ...) remain the file home for now.
let _sbClient = null;
const _getSupabase = async () => {
  if (_sbClient) return _sbClient;
  const mod = await import("./supabase");
  _sbClient = mod.default;
  return _sbClient;
};
...
export const getCurrentUserWithoutStream = async () => {
  ...
  // the user has no Appwrite session, so calling account.get() throws
  // "User (role: guests) missing scope (account)". Edit-profile flows
  // (username / avatar / banner / role) call this helper after every
  // update to refresh the user object — the Appwrite throw made the
  // updates LOOK like they failed even though the Supabase write
  // succeeded. Delegate to the Supabase-aware fetcher in that mode so
  // the refresh actually returns a hydrated user.
  if (USE_SUPABASE_AUTH) {
    const { getCurrentSupabaseUser } = await import("./supabase-auth");
...
export async function updateUsername(documentId, newUsername) {
  try {
    if (USE_SUPABASE_AUTH) {
      // Route through the rate-limited RPC instead of a direct table
      // update. The RPC enforces 2 changes / 30 days server-side,
      // validates characters (no emoji), and writes an audit row in the
      // same transaction as the username change. See
      // supabase/migrations/2026-05-08_username_rate_limit.sql.
      const sb = await _getSupabase();
      const { data, error } = await sb.rpc("update_username", {
        p_new_username: newUsername,
      });
      if (error) throw new Error(error.message || "Failed to update username");
      if (!data?.ok) {
        // Surface the structured error to the UI. The RPC returns a
        // human-readable `message` field for known errors; fall back
        // to the error code if it's missing.
        const friendly = data?.message || _humanizeUsernameError(data?.error);
      ...
      return { $id: documentId, username: data.username, changes_remaining: data.changes_remaining };
    }
...
// Last-resort error code → user-readable mapping. The RPC normally
// returns a `message` field, but if a future error code lands without
// one, fall back to this map so users never see a raw code.
function _humanizeUsernameError(code) {
  switch (code) {
    ...
    case "username_invalid_chars":
      return "Username can only contain letters, numbers, spaces, periods, hyphens, and underscores.";
    case "username_unchanged":
      return "New username is the same as the current one.";
    case "rate_limit_exceeded":
      return "You can only change your username 2 times every 30 days.";
    case "not_authenticated":
      return "Sign in to change your username.";
...
// Read the user's current rate-limit status — used by the edit-profile
// screen to show "X changes remaining this month" and disable the Save
// button when the cap is hit, without having to attempt the update
// just to discover the limit is exhausted.
export async function getUsernameChangeStatus() {
  if (!USE_SUPABASE_AUTH) {
    // Legacy Appwrite path has no rate limit — return an "unlimited"
    // shape so the UI doesn't gate anything.
    return { ok: true, changes_used: 0, changes_remaining: 999, max_per_window: 999, window_days: 30 };
  }
  try {
    const sb = await _getSupabase();
    const { data, error } = await sb.rpc("username_change_status");
    if (error) throw new Error(error.message || "Failed to read username status");
    return data;
  } catch (error) {
    // Soft-fail — if the status check breaks, return a permissive shape
    // so the user can still attempt a change (the RPC itself will
    // enforce the limit if it's actually hit).
    console.warn("[updateUsername] status read failed:", error?.message);
    return { ok: true, changes_used: 0, changes_remaining: 2, max_per_window: 2, window_days: 30 };
  }
}
  ...
  try {
    ...
    if (USE_SUPABASE_AUTH) {
      ...
      const { error } = await sb
        .from("profiles")
        .update({ roles: nextRoles, badge_expiration: badgeExpiration })
        .eq("id", documentId);
      if (error) throw new Error(error.message || "Failed to update role in Supabase");
      invalidateUserCache(documentId);
      return { $id: documentId, roles: nextRoles, badgeExpiration };
    }
    const response = await databases.updateDocument(appwriteConfig.databaseId, appwriteConfig.userCollectionId, documentId, {
      roles: nextRoles,
      badgeExpiration,
...
export async function updateBio(documentId, bio) {
  try {
    if (USE_SUPABASE_AUTH) {
      const sb = await _getSupabase();
      const { error } = await sb.from("profiles").update({ bio: typeof bio === "string" ? bio : "" }).eq("id", documentId);
      if (error) throw new Error(error.message || "Failed to update bio in Supabase");
      invalidateUserCache(documentId);
      return { $id: documentId, bio };
    }
...
// Hotfix (May 2026, USE_SUPABASE_AUTH=true): users no longer have an
// Appwrite session, so `storage.createFile` fails with
// "User (role: guests) missing scopes (account)" on every avatar/banner
// upload. Route uploads to Supabase Storage instead — same auth context
// as the rest of the app (RLS-aware), and the public buckets give us a
// public URL that works without any per-user session.
//
// Required Supabase Storage setup (one-time, in dashboard):
//   1. Create bucket `avatars` — Public: ON
//   2. Create bucket `banners` — Public: ON
//   3. RLS on storage.objects: allow authenticated INSERT, public SELECT
//
// Old Appwrite `getFilePreview` URLs already in `profiles.avatar_url` /
// `profiles.banner_url` keep working for users who set them pre-migration
// (those bucket files still exist with public read). Only NEW uploads
// route through Supabase Storage.
async function uploadProfileImageToSupabaseStorage(file, { bucket, userId, maxWidth }) {
  ...
  try {
    // Path layout: <userId>/<ts>.webp. Folder per user lets us scope
    // Storage RLS to "user can only write under their own folder" if
    // we ever tighten policies later.
    const filename = `${userId}/${Date.now()}.webp`;
    // Read as ArrayBuffer, NOT Blob. React Native's polyfilled Blob
    // produces 0-byte uploads with @supabase/supabase-js — well-known
    // gotcha. ArrayBuffer is the supported binary payload type for
    // RN + Supabase Storage.
    const fileResponse = await fetch(webp.uri);
    ...
    const { error: uploadErr } = await sb.storage
      ...
      .upload(filename, fileArrayBuffer, {
        ...
        upsert: false, // unique filename — never overwrite
      });
...
export async function updateAvatar({ file, userId, previousAvatar }) {
  try {
    // Under USE_SUPABASE_AUTH=true, the user has no Appwrite session, so
    // Appwrite's storage.createFile would fail with "missing scopes
    // (account)". Upload to Supabase Storage instead — the user's
    // Supabase session authorizes the write via Storage RLS. Skip the
    // Appwrite delete of the previous avatar too (no Appwrite session to
    // authorize the delete; the old file just becomes orphaned in
    // Appwrite — fine).
    if (USE_SUPABASE_AUTH) {
      const avatarUrl = await uploadProfileImageToSupabaseStorage(file, { bucket: "avatars", userId, maxWidth: 500 });
      const sb = await _getSupabase();
      const { error } = await sb.from("profiles").update({ avatar_url: avatarUrl }).eq("id", userId);
      if (error) throw new Error(error.message || "Failed to update avatar in Supabase");
      invalidateUserCache(userId);
      return { $id: userId, avatar: avatarUrl, avatar_url: avatarUrl };
    }
...
export async function updateBanner({ file, userId, previousBanner }) {
  try {
    // Under USE_SUPABASE_AUTH=true, the user has no Appwrite session.
    // Skip Appwrite's deleteFile + createFile (both would 401 with
    // "missing scopes (account)") and route the upload + the profile
    // write through Supabase. Old files left in Appwrite become
    // orphaned — acceptable, they'll get cleaned up later.
    if (USE_SUPABASE_AUTH) {
      const bannerUrl = await uploadProfileImageToSupabaseStorage(file, {
        bucket: "banners",
        userId,
        maxWidth: Math.max(PROFILE_BANNER_UPLOAD_WIDTH, file?.width || 0),
      });
      const sb = await _getSupabase();
      const { error } = await sb.from("profiles").update({ banner_url: bannerUrl }).eq("id", userId);
      if (error) throw new Error(error.message || "Failed to update banner in Supabase");
      invalidateUserCache(userId);
      return { $id: userId, banner: bannerUrl, banner_url: bannerUrl };
    }
