...
// Inline-comment dual-write was originally wired up here, but the
// Supabase chapter_inline_comments schema requires a thread_id pointing
// at chapter_inline_comment_threads (offset-based addressing). Mobile
// uses anchorKey + ordinal instead, and there's no clean translation
// from one to the other without rendering chapter content. Until a
// dedicated anchor→offset migration lands, mobile inline comments stay
// Appwrite-only — see lib/books-dual-write.js for the longer note.
...
export const BookInlineCommentsService = {
  ...
  async fetchAllChapterThreads({ bookChapterId }) {
    // Inline comments still live on Appwrite (anchorKey + ordinal model).
    // Post-Supabase-auth cutover the user has no Appwrite session, so the
    // listDocuments call below would 401 on every chapter open. Return
    // empty silently — feature degrades to "no inline threads" until the
    // anchor→offset migration lands. See top-of-file note.
  ...
  async createInlineComment({ bookChapterId, anchor, comment, commentOwner, commentOwnerDocument }) {
    ...
    try {
      ...
      // Inline-comment dual-write deliberately skipped — anchor model
      // mismatch (see books-dual-write.js).
  ...
  async createReplyComment({ comment, commentOwner, bookChapterInlineComment, commentOwnerDocument }) {
    ...
    try {
      ...
      // Inline-reply dual-write deliberately skipped — anchor model
      // mismatch (see books-dual-write.js).
