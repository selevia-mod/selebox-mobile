...
export const fetchUserEarnings = async (_unusedAccountId, monthYear) => {
  ...
  return {
    total: lifetimeTotalMinor / 100,
    totalEarningsThisMonth: totalEarningsThisMonthMinor / 100,
    // Surface available + pending separately so the Payments screen can
    // show "Remaining" (available) and "Pending" tiles. Writers were
    // panicking because new credits land in `pending` for the hold
    // period and the old UI rolled them into a single number.
    available: (Number(balance?.available_php_minor) || 0) / 100,
    pending: (Number(balance?.pending_php_minor) || 0) / 100,
    breakdown: {
      posts: breakdownMinor.posts / 100,
      clips: breakdownMinor.clips / 100,
      video: breakdownMinor.video / 100,
