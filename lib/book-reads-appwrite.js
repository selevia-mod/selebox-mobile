...
import { dualWriteBookRead, dualWriteChapterRead } from "./books-dual-write";
import { createTtlCache } from "./utils/createTtlCache";

const OFFLINE_READS_KEY = "book-reads-offline-v1";

// Per-book read-stat cache. Hit hard from BookCard / BookCatalogCard /
// BookLibraryCard / BookInfoStats / BooksDiscover — every card on the Books
// tab fires fetchBookRead on mount, and same book stats are looked up from
// many surfaces in a session. 60s TTL is fine; read counts are not real-time.
//
// We cache `null` too — the original code logged "No read record found" 37+
// times when scrolling fresh books, costing both a network round-trip AND a
// noisy console log per absent record. Caching the negative answer makes
// repeat lookups instant.
const BOOK_READ_CACHE = createTtlCache({ ttlMs: 60 * 1000, maxEntries: 500 });

// Sentinel for caching "we checked, no record exists." Without this, a `null`
// cache value is indistinguishable from "not yet cached," and the lookup
// would fall through to a real query every time.
const NULL_VALUE = Symbol("BookReadCache.null");

export const invalidateBookReadCache = (bookId) => {
  if (bookId) BOOK_READ_CACHE.delete(bookId);
};

/**
 * Batched read-stats fetch for a list of book IDs. ONE query instead of N.
 * Returns a Map keyed by bookId — present means there's a stats doc, absent
 * means no record exists (which the caller should treat as zero counts).
 *
 * Use from list-level loaders (books.jsx feed, BooksRanking, etc.) so each
 * card doesn't fire its own fetchBookRead on mount.
 */
export const fetchBookReadsByIds = async (bookIds = []) => {
  const result = new Map();
  const unique = Array.from(new Set(bookIds.filter(Boolean)));
  if (unique.length === 0) return result;

  // Cached entries don't need to round-trip.
  const toFetch = [];
  for (const id of unique) {
    const cached = BOOK_READ_CACHE.get(id);
    if (cached === undefined) {
      toFetch.push(id);
    } else if (cached !== NULL_VALUE) {
  ...
  try {
    const response = await databases.listDocuments(appwriteConfig.databaseId, appwriteConfig.booksReadsCollectionId, [
      Query.equal("book", toFetch),
      Query.limit(toFetch.length),
    ]);

    const seen = new Set();
    for (const doc of response.documents || []) {
      // The `book` field is a relationship — could be a string ID or an
      // expanded object depending on the schema. Handle both.
      const docBookId = typeof doc.book === "string" ? doc.book : doc.book?.$id;
      if (!docBookId) continue;
      result.set(docBookId, doc);
      BOOK_READ_CACHE.set(docBookId, doc);
      seen.add(docBookId);
    }

    // Cache "no record" for every bookId we asked about but didn't see in
    // the response. Stops 30+ "No read record found" follow-ups on next
    // scroll past the same books.
    for (const id of toFetch) {
      if (!seen.has(id)) BOOK_READ_CACHE.set(id, NULL_VALUE);
    }
  } catch (err) {
    console.error("fetchBookReadsByIds error:", err?.message || err);
  }
...
export const BookReadService = {
  ...
  readBookChapter: async ({ userId, bookId, chapterId }) => {
    try {
      ...
      // Dual-write to Supabase. Mirrors BOTH:
      //   • book_reads — the (book_id, user_id, last_read_at, last_chapter_id)
      //     row web's readBookChapter writes. Powers "Continue Reading".
      //   • chapter_reads — the per-(user, chapter) row with read_count.
      //     Created in migration_chapter_reads.sql; tracks which user has
      //     read which chapters and how many times they've re-read.
      // Best-effort: if any mirror is missing (user/book/chapter not yet
      // resolved via legacy_appwrite_id), the helper skips and the backfill
      // catches it later.
      try {
        await dualWriteBookRead({ bookAppwriteId: bookId, userAppwriteId: userId, chapterAppwriteId: chapterId });
      } catch (sbErr) {
        console.error("[book-reads-appwrite] readBookChapter book_reads dual-write skipped:", sbErr?.message);
      }
      try {
        await dualWriteChapterRead({ chapterAppwriteId: chapterId, userAppwriteId: userId });
      } catch (sbErr) {
        console.error("[book-reads-appwrite] readBookChapter chapter_reads dual-write skipped:", sbErr?.message);
      }
    } catch (err) {
      ...
      for (const [bookId, bookEntry] of Object.entries(userEntries)) {
        ...
        for (const [chapterId, chapterEntry] of Object.entries(chapters)) {
          ...
          try {
            ...
            synced += count;
            removeOfflineChapter({ userId, bookId, chapterId });
            // book_reads dual-write happens once per book at the end of
            // the inner loop (composite-PK (user_id, book_id) — N
            // per-chapter upserts would overwrite the same row).
            // chapter_reads is per-(user, chapter) though, so it MUST
            // mirror inside this loop with the accumulated offline count
            // — otherwise re-reads collected offline would never increment
            // the Supabase read_count. Best-effort.
            try {
              await dualWriteChapterRead({ chapterAppwriteId: chapterId, userAppwriteId: userId, incrementBy: count });
            } catch (sbErr) {
              console.error("[book-reads-appwrite] syncOfflineReads chapter_reads dual-write skipped:", sbErr?.message);
            }
          } catch (err) {
        ...
        if (bookEntry?.lastChapterId) {
          ...
          // One Supabase dual-write per book using the latest chapter.
          // book_reads is composite-PK (user_id, book_id), so writing once
          // with the most recent lastChapterId converges to the same final
          // state as the per-chapter writes the legacy code did to
          // booksChaptersReadsCollection — without N extra round-trips.
          try {
            await dualWriteBookRead({
              bookAppwriteId: bookId,
              userAppwriteId: userId,
              chapterAppwriteId: bookEntry.lastChapterId,
            });
          } catch (sbErr) {
            console.error("[book-reads-appwrite] syncOfflineReads Supabase dual-write skipped:", sbErr?.message);
          }
        }
      }
  ...
  fetchBookRead: async ({ bookId }) => {
    try {
      ...
      // Cache check — returns either a real doc or the sentinel `NULL_VALUE`
      // for "we already checked, this book has no record." Both spare the
      // round-trip on subsequent renders.
      const cached = BOOK_READ_CACHE.get(bookId);
      if (cached !== undefined) {
       ...
       *
       * Cache the negative result so we don't re-query on every card mount.
       * Also dropped the per-call console.log here — with ~37 visible cards
       * on a fresh-data Books tab, that single log was firing dozens of
       * times per scroll and noticeably slowing the bridge.
       */
      if (response.total === 0) {
        BOOK_READ_CACHE.set(bookId, NULL_VALUE);
        return null;
      }
      ...
      BOOK_READ_CACHE.set(bookId, bookReadDoc);
      return bookReadDoc;
    } catch (err) {
