...
export class MessagesService {
  ...
  static async countChatsWithUnreadMessages({ userId }) {
    try {
      ...
      // Used to drive the chat unread badge in the header. Re-throwing here
      // breaks the badge silently with an unhandled promise rejection. Return
      // 0 instead — wrong number is recoverable, missing badge is fine, but
      // a thrown promise mid-render is not.
      logger.error("MessagesService", "countChatsWithUnreadMessages failed", error);
      return 0;
    }
  }
}
