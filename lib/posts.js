...
// UUID-aware. Mirrors the routing pattern fetchPostLikes / getPost
// already use:
//   • Supabase-native post (UUID id) → query the `reactions` table via
//     getMyReaction. Returns Appwrite-shaped { documents, total } so
//     consumers (PostInformation's syncLike, etc.) don't care which
//     backend the post lives on.
//   • Legacy Appwrite post (20-char hex id) → original Appwrite call.
//
// Why this matters: PostInformation has its own _supabase flag check
// that routes Supabase posts through getMyReaction directly, but the
// flag occasionally goes missing as posts pass through code paths that
// don't run through adaptSupabasePostToAppwriteShape. When that
// happens the component falls through to the legacy getPostLike below
// and 401s against Appwrite (no Appwrite session for that post). UUID
// detection here is the safety net so the lib layer always picks the
// right backend regardless of which adapter the caller used.
const _UUID_RE_POST_LIKE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
export const getPostLike = async ({ postId, likeOwner }) => {
  if (typeof postId === "string" && _UUID_RE_POST_LIKE.test(postId)) {
    try {
      const { getMyReaction } = await import("./reactions-supabase");
      const emoji = await getMyReaction({ targetType: "post", targetId: postId });
      if (!emoji) return { documents: [], total: 0 };
      // Appwrite-shaped envelope. The legacy syncLike path reads
      // `existing.documents[0].$id` to feed deletePostLike; we
      // synthesize a stable composite id (`reaction:postId:owner`)
      // so deletePostLike below can detect the prefix and route
      // through removeMyReaction instead of an Appwrite delete.
      return {
        documents: [
          {
            $id: `reaction:${postId}:${likeOwner || ""}`,
            postId,
            likeOwner,
            emoji,
            _supabase: true,
      ...
      console.warn("[posts] getPostLike (supabase) failed:", error?.message);
...
// UUID-aware. Same routing rules as getPostLike — Supabase post IDs go
// through the reactions table via setReaction (which handles
// add/change atomically through the submit_reaction RPC).
export const createPostLike = async ({ postId, likeOwner }) => {
  if (typeof postId === "string" && _UUID_RE_POST_LIKE.test(postId)) {
    const { setReaction } = await import("./reactions-supabase");
    await setReaction({ targetType: "post", targetId: postId, emoji: "heart" });
    // Mirror Appwrite's createDocument response shape so the legacy
    // call site (PostInformation syncLike) doesn't need to branch.
    return { $id: `reaction:${postId}:${likeOwner || ""}`, _supabase: true };
  }
...
// UUID-aware. The composite id we synthesize in getPostLike for Supabase
// reactions has the `reaction:` prefix; detect it here and route to
// removeMyReaction instead of trying to delete an Appwrite doc that
// doesn't exist. Plain Appwrite hex ids continue to the legacy path.
export const deletePostLike = async ({ postLikeId }) => {
  if (typeof postLikeId === "string" && postLikeId.startsWith("reaction:")) {
    // Format: reaction:<postId>:<ownerId>
    const parts = postLikeId.split(":");
