...
export class VideosServiceSupabase {
  ...
  async fetchVideos({ userId, lastId, category, limit = 20, status, offset }) {
    ...
    if (status) {
      // Hotfix (May 2026): the actual canonical live status in the
      // current videos schema is 'published' (3151 rows), not 'ready'
      // (only 15 rows). The schema flipped after the comment block
      // below was written, but this translation kept routing every
      // 'published' query to 'ready' and returning ~0 results. Pass
      // the status through unchanged.
      //
      // Previous (stale) note: "Mobile call sites pass status:
      // 'published' (Appwrite-era token). The Supabase + web schema
      // settled on 'ready' as the canonical live state." — no longer
      // accurate; published is canonical now.
...
export const createNewVideo = async ({
  ...
  // ID handling — UploadVideo.jsx generates the videoID via
  // react-native-appwrite's ID.unique(), which returns a 20-char hex
  // string (NOT a UUID). It uses that hex as the Bunny storage path
  // and passes it here as `ID`. Earlier we forwarded it directly into
  // the `id` column, but `videos.id` is UUID type — Postgres rejected
  // the hex with "invalid input syntax for type uuid" and every
  // upload failed at the DB write step.
  //
  // Correct routing:
  //   • Hex string (Appwrite-shaped) → store in legacy_appwrite_id,
  //     let Supabase generate a fresh UUID for `id`.
  //   • UUID string (web-native or future native flow) → use as-is
  //     in the `id` column.
  //   • undefined / null → Supabase generates everything.
  const insertPayload = {
    title,
    description,
  ...
  if (ID) {
    if (UUID_RE.test(ID)) {
      ...
      // Appwrite hex — preserve the linkage for cross-backend lookups
      // (Bunny storage path, dual-write reconciliation, etc.) but let
      // Supabase generate the canonical UUID.
      insertPayload.legacy_appwrite_id = ID;
    }
  }
