...
const COMMENT_SELECT = `id, user_id, parent_id, post_id, video_id, body, image_url, created_at, deleted_at,
...
// uses internally, ready for a tree-render. Soft-deleted rows are
// filtered out server-side via the deleted_at IS NULL predicate.
//
...
// Counts comments on a single post. Excludes soft-deleted. Used by
// PostInformation's stats row when the home feed wants a cheap refresh
// after the user adds/removes a comment.
export const fetchCommentCount = async ({ postId, videoId } = {}) => {
  if (!postId && !videoId) return 0;
  let query = supabase.from("comments").select("*", { count: "exact", head: true }).is("deleted_at", null);
...
export const fetchCommentCountsForPosts = async (postIds = []) => {
  ...
  const { data, error } = await supabase.from("comments").select("post_id").in("post_id", postIds).is("deleted_at", null);
  if (error) {
