...
const COMMENT_SELECT = `id, user_id, parent_id, post_id, video_id, body, image_url, created_at,
  profiles!user_id(id, username, avatar_url, is_guest, role, legacy_appwrite_id)`;
...
export const addComment = async ({ postId, videoId, parentId = null, body = "", imageUrl = null }) => {
  ...
  // Route through submit_comment RPC — RLS rejects direct `.insert()`
  // when auth.uid() is null (mobile anon session) AND when web sessions
  // expire mid-page. The RPC validates the actor against profiles and
  // bypasses the RLS check via SECURITY DEFINER.
  const { data: rpcResult, error: rpcErr } = await supabase.rpc("submit_comment", {
    p_actor_id: me.id,
  ...
  if (!rpcResult?.id) throw new Error(rpcResult?.error || "submit_comment returned no id");

  // Re-fetch the inserted row in the canonical SELECT shape so the caller
  // (PostCommentModal) can append it without a second adaptation pass.
  // SELECT is RLS-permitted, so this read works under either auth state.
  const { data, error } = await supabase
    .from("comments")
    .select(COMMENT_SELECT)
    .eq("id", rpcResult.id)
  ...
  // image_url isn't part of submit_comment yet; if the caller passed one,
  // patch it in via UPDATE. The RLS UPDATE policy on comments still
  // requires auth.uid() = user_id, so this only works for web users.
  // Mobile commenters with images currently lose the image on the
  // Supabase mirror. Tracked separately — extend submit_comment to
  // accept p_image_url when needed.
  if (data && imageUrl) {
    try {
      await supabase.from("comments").update({ image_url: imageUrl }).eq("id", rpcResult.id);
      data.image_url = imageUrl;
    } catch (_) { /* swallow — text still went through */ }
  }
  return data;
};
...
export const adaptSupabaseCommentToAppwriteShape = (row, { replies = [], likes = [], myReaction = null } = {}) => {
  ...
  return {
    ...
    commentOwner: {
      ...
      // Expand role flags so UserRoleBadgeIcons next to comment author
      // names picks up the verified seal.
      ...expandProfileRoleFlags(profile),
    },
...
// ─────────────────────────────────────────────────────────────────────────
// Realtime — subscribe to comments INSERT/UPDATE/DELETE for a single post
// or video. Mobile + web both call this on modal open; both unsubscribe
// on close. Mirrors the chat thread realtime pattern.
//
// Why the per-mount channel-name suffix:
//   Supabase Realtime returns the SAME channel object on repeated
//   `supabase.channel("comments-post:X")` calls. If two modals subscribe
//   in succession (open → close → reopen), the second .on() throws
//   "cannot add postgres_changes callbacks ... after subscribe()". The
//   counter suffix gives every subscribe call its own fresh channel
//   so the registry never collides. Same trick we use in
//   subscribeToConversation in messages-supabase.js.
// ─────────────────────────────────────────────────────────────────────────

let __commentsChannelSeq = 0;

export const subscribeToComments = ({ postId, videoId, onInsert, onUpdate, onDelete } = {}) => {
  if (!postId && !videoId) return () => {};
  const filterCol = postId ? "post_id" : "video_id";
  const filterId = postId || videoId;
  ...
  return () => {
    // Defensive removeChannel — same pattern as the chat thread.
    // Some SDK versions throw if the channel never finished subscribing
    // (e.g., the user closed the modal before SUBSCRIBED fired).
    try {
      if (channel) supabase.removeChannel(channel);
    } catch (error) {
      console.log("[comments-supabase] channel cleanup failed:", error?.message);
    }
  };
};
