...
export const createTtlCache = ({ ttlMs = DEFAULT_TTL_MS, maxEntries = DEFAULT_MAX_ENTRIES } = {}) => {
  ...
  return {
    ...
    // Map.prototype.has parity. Honors TTL — an expired entry counts
    // as absent (and is also evicted as a side effect, same as get()).
    // Callers that already use Map's `has` pattern can swap to a TTL
    // cache without code changes.
    has(key) {
      const entry = store.get(key);
