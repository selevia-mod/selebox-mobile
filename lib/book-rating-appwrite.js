...
import { dualWriteBookRating } from "./books-dual-write";
...
export class BookRatingService {
  ...
  static async createRating({ bookId, userId, rating }) {
    try {
      ...
      // Dual-write to Supabase. book_ratings has composite PK (book_id,
      // user_id) so this also handles re-rating (an upsert).
      try {
        await dualWriteBookRating({ bookAppwriteId: bookId, userAppwriteId: userId, rating });
      } catch (sbErr) {
        console.log("[book-rating-appwrite] createRating Supabase dual-write skipped:", sbErr?.message);
      }
      return res;
    } catch (err) {
