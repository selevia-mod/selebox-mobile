...
import { dualWriteSubmitFollow, dualWriteSubmitUnfollow } from "./posts-dual-write";
...
export class FollowService {
  static async followUser({ followerId, followingId }) {
    try {
      ...
      // Mirror to Supabase via the SECURITY DEFINER RPC. Mobile users
      // can't directly insert to public.follows under the anon key —
      // RLS rejects them. The RPC bypasses RLS and the trigger
      // `notify_on_follow` fires automatically, so the recipient also
      // gets a bell notification on web. Best-effort: never blocks the
      // Appwrite write.
      try {
        await dualWriteSubmitFollow({ followerAppwriteId: followerId, followingAppwriteId: followingId });
      } catch (sbErr) {
        console.log("[follows-appwrite] follow Supabase mirror skipped:", sbErr?.message);
      }
      return res;
    } catch (error) {
  ...
  static async unfollowUser({ followerId, followingId }) {
    try {
      ...
      try {
        await dualWriteSubmitUnfollow({ followerAppwriteId: followerId, followingAppwriteId: followingId });
      } catch (sbErr) {
        console.log("[follows-appwrite] unfollow Supabase mirror skipped:", sbErr?.message);
      }
    } catch (error) {
