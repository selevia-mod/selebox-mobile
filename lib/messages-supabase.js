...
//   Every function here assumes a Supabase auth session exists — `currentUser`
//   is read from `supabase.auth.getUser()`. Callers should gate the chat tab
//   on `isLogged === true` so we never enter this module unauthenticated.
...
export const getOrCreate1to1Conversation = async (otherUserId) => {
  ...
  const { data: created, error } = await supabase
    .from("conversations")
    .insert({ user_a: me.id, user_b: otherUserId, is_group: false, created_by: me.id })
    .select()
    .single();
  if (error) throw error;
  return created;
