...
// 60s cache for follower / following counts. Profile.jsx fires both on
// every focus (useFocusEffect with [user] deps), so navigating to a profile
// and back used to fan out 4 supabase queries per visit (2 per count: live
// + snapshot). Counts move slowly and a 60s TTL feels live in the UI while
// killing the burst. followUser/unfollowUser invalidate the affected pair
// below so optimistic updates never see stale numbers.
const FOLLOW_COUNTS_CACHE = createTtlCache({ ttlMs: 60 * 1000, maxEntries: 500 });
const followersCountKey = (uuid) => `followers::${uuid}`;
const followingCountKey = (uuid) => `following::${uuid}`;
const invalidateCountsForPair = async (followerRawId, followingRawId) => {
  try {
    const [fromUuid, toUuid_] = await Promise.all([toUuid(followerRawId), toUuid(followingRawId)]);
    if (fromUuid) FOLLOW_COUNTS_CACHE.delete(followingCountKey(fromUuid));
    if (toUuid_) FOLLOW_COUNTS_CACHE.delete(followersCountKey(toUuid_));
  } catch (_) {
    // Best-effort invalidation — TTL will catch up within 60s anyway.
  }
};
...
export class FollowServiceSupabase {
  ...
  static async followUser({ followerId, followingId }) {
    try {
      ...
      FOLLOW_STATUS_CACHE.set(followKey(followerId, followingId), true);
      // Drop cached counts for both ends of the edge so the next stat
      // fetch reflects the fresh number instead of waiting up to 60s.
      invalidateCountsForPair(followerId, followingId);
      return data;
    } catch (error) {
  ...
  static async unfollowUser({ followerId, followingId }) {
    try {
      ...
      // Drop cached counts so the next stat fetch reflects the unfollow.
      invalidateCountsForPair(followerId, followingId);
    } catch (error) {
  ...
  // Hotfix (May 6, 2026) — under USE_SUPABASE_AUTH=true, the Supabase
  // lazy-claim trigger destroys follow rows on each migrated user's
  // first sign-in (mechanism still under investigation; FK has ON
  // UPDATE CASCADE so it isn't the trigger UPDATE alone). Until that's
  // root-caused and fixed, we display the larger of (live count, frozen
  // snapshot) so user-visible counts never drop. The snapshot lives in
  // `public.follower_count_snapshot` — populated from the current
  // follows table at deploy time (see hotfix SQL alongside this OTA).
  // As users gain new followers, the live count naturally grows past
  // the snapshot and becomes the displayed number; the snapshot only
  // matters as a floor.
  static async getFollowersCount({ userId }) {
    try {
      ...
      const cacheKey = followersCountKey(uuid);
      const cached = FOLLOW_COUNTS_CACHE.get(cacheKey);
      ...
      const snapPromise = supabase
        .from("follower_count_snapshot")
        .select("followers")
        .eq("user_id", uuid)
        .maybeSingle();

      const [liveRes, snapRes] = await Promise.all([livePromise, snapPromise]);
      if (liveRes.error) throw liveRes.error;
      const live = liveRes.count || 0;
      const snapshot = snapRes?.data?.followers ?? 0;
      const result = Math.max(live, snapshot);
      FOLLOW_COUNTS_CACHE.set(cacheKey, result);
      return result;
    } catch (error) {
  ...
  static async getFollowingCount({ userId }) {
    try {
      ...
      const cacheKey = followingCountKey(uuid);
      const cached = FOLLOW_COUNTS_CACHE.get(cacheKey);
      if (typeof cached === "number") return cached;
      ...
      if (liveRes.error) throw liveRes.error;
      const live = liveRes.count || 0;
      const snapshot = snapRes?.data?.following ?? 0;
      const result = Math.max(live, snapshot);
      FOLLOW_COUNTS_CACHE.set(cacheKey, result);
      return result;
    } catch (error) {
  ...
  static async getFollowers({ userId, limit, cursor }) {
    try {
      ...
      // Hydrate the follower's profile so the Followers list shows
      // username + avatar instead of blank placeholder cards.
      let q = supabase
        .from("follows")
        .select(`
          follower_id, following_id, created_at,
          follower:profiles!follows_follower_id_fkey (
            id, username, avatar_url, legacy_appwrite_id, role
          )
        `)
        .eq("following_id", uuid)
      ...
      const documents = (data || []).map((row) => {
        const followerProfile = row.follower || {};
        return {
          ...
          // Hydrated profile of the follower (the user who is following YOU).
          followerUser: {
            $id: followerProfile.legacy_appwrite_id || followerProfile.id,
            id: followerProfile.id,
            username: followerProfile.username,
            name: followerProfile.username,
            avatar: followerProfile.avatar_url,
            avatar_url: followerProfile.avatar_url,
            legacy_appwrite_id: followerProfile.legacy_appwrite_id,
            role: followerProfile.role || "user",
          },
        };
      });
  ...
  static async getFollowing({ userId, limit, cursor }) {
    try {
      ...
      // Hydrate the followed user's profile in the same query — without
      // this the consumer screen renders blank placeholder cards (gray
      // circle + "Follow" button) because the row only carries IDs.
      // Foreign-key shorthand `profiles!follows_following_id_fkey`
      // pulls username/avatar/role in a single round-trip.
      let q = supabase
        .from("follows")
        .select(`
          follower_id, following_id, created_at,
      ...
      const documents = (data || []).map((row) => {
        ...
        return {
          ...
          following_id: row.following_id,
          created_at: row.created_at,
          // Hydrated profile of the user being followed — both legacy
          // shape and Supabase shape so old and new consumers both work.
          followedUser: {
            $id: followed.legacy_appwrite_id || followed.id,
            id: followed.id,
            username: followed.username,
            name: followed.username,
            avatar: followed.avatar_url,
            avatar_url: followed.avatar_url,
            legacy_appwrite_id: followed.legacy_appwrite_id,
            role: followed.role || "user",
          },
        };
      });
