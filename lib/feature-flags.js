 ...
 * MOMENTS_VIEWER_SWR
 *
 * When TRUE, the moments viewer (app/(story)/story-viewer.jsx) seeds its
 * user-stack state from the Redux story cache (storyCache.byViewerId) on
 * mount, so tapping a tile renders the cube INSTANTLY from cached data.
 * The network fetch then runs in the background and replaces the data
 * silently when fresh stories arrive. When FALSE, the legacy behavior
 * runs: every viewer mount blocks on a network fetch and shows a brief
 * spinner placeholder.
 *
 * Trade-offs the flag covers:
 *   - Stale flash: cached stories may have been deleted server-side.
 *     The existing availabilityChecked effect catches this and re-fetches
 *     with forceNetwork, but you'll see a tick of the deleted story.
 *   - Discover-only taps still spin: the cache is viewer-scope only
 *     (own + followings). Tapping a discover creator with no cache
 *     entry falls back to the legacy network-first path, which is fine.
 *   - Cache TTL still applies (isViewerStoryCacheFresh — 5min by
 *     default). Past TTL we transparently fall back to the spinner so
 *     users on day-old caches don't see ancient stories.
 *
 * Rollback: flip back to FALSE — the legacy network-first code path is
 * still present and there's no data shape change.
 *
 * Status: ACTIVATED. Originally shipped DARK (FALSE) in the OTA that
 * introduced it (task #161) so we could verify no regressions in the
 * legacy network-first path. After several OTAs of soak time with no
 * regressions reported, flipped TRUE to activate zero-flash for all
 * users. The legacy path is still present as the cache-miss fallback
 * (cold viewer, expired TTL, discover-only tap) — only the cached
 * happy-path is changed.
 *
 * If you ever need to roll back: flip to FALSE again, no other code
 * changes required. The seedFromCache helper and race-protected
 * refresh handle both states cleanly.
 */
export const MOMENTS_VIEWER_SWR = true;
