...
export const StarServiceSupabase = {
  ...
  earnStar: async (_userId) => {
    try {
      ...
      if (data?.ok === false) {
        // Daily cap or not-signed-in case. Surface to caller via the
        // `error` field; the UI alerts on Alert.alert(err.message) so
        // make the message friendly.
        const msg =
