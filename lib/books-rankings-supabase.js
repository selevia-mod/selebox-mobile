...
// content_rating is required for the BookRankingCard's "Rated PG / Rated 18 /
// SSPG" tag. Forgetting it here was the regression that flipped every
// ranked book back to the default "Rated PG" fallback in mapBookRow even
// though the column itself was correctly backfilled in the database.
const BOOK_SELECT = `
...
// Tag-based content rating inference — duplicates the helper in
// `books-supabase.js`. Without this, the rankings shelf showed every
// book as "Rated PG" (BookRankingCard's hardcoded fallback) because
// the mapper here never included `contentRating` at all. We mirror
// the same regex / fallback contract so a Hot Romance / Dark Romance
// / SSPG book on the ranking lands as "Rated 18" without the author
// having to set the explicit column.
const MATURE_TAG_PATTERNS = /\b(hot|dark|mature|adult|smut|erotic|erotica|18\+?|nsfw|explicit|sspg|spg|r18)\b/i;
...
const mapBookRow = (row) => {
  ...
  return {
    ...
    // Content rating — three-tier: explicit column → tag inference
    // → "Rated PG" default. Matches books-supabase.js so consumers
    // (book-info, ranking card, catalog card) all render a consistent
    // pill regardless of which mapper provided the row.
    contentRating: row.content_rating || inferContentRatingFromTags(row.tags, row.genre) || "Rated PG",
